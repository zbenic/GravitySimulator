...
class PhysicalObject:
    def __init__(self, object_id, position, diameter, velocity_vec, color, total_object_count, timestep):
        ...
        self.accelerations = np.zeros(shape=(total_object_count, 2))
    ...
    def _get_object_acceleration(self, celestial_objects):
        ...
        for celestial_object in celestial_objects:
            ...
            celestial_object.accelerations[id] = celestial_object.mass * connecting_vec *\
                                                 (np.linalg.norm(connecting_vec)) ^ (-3)
            total_object_acc += celestial_object.accelerations[id]
